...
from sample_identify_and_merge_cross_page_tables import identify_cross_page_tables
import markdownify 
...
class AzureAIDocumentIntelligenceParser(BaseBlobParser):
    ...
    def _generate_docs_single(self, result: Any) -> Iterator[Document]:
        yield Document(page_content=result.content, metadata={})
         ...
         # table
        for table in identify_cross_page_tables(result, self.table_max_rows):
            yield Document(
                page_content=markdownify.markdownify(table["content"], heading_style="ATX") ,  # json object
                metadata={
                    #"footnote": table.footnotes,
                    #"caption": table.caption,
                    "header_text" : table["header_text"],
                    "page": table["table_page"],
                    "row_count": table["non_header_row_count"],
